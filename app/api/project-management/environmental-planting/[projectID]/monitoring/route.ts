...
export async function PUT(
  ...
  try {
    ...
    const { monitoringPlotsUrl, monitoringDataUrl, monitoringAdditionalFilesUrls, monitoringAdditionalFilesDescription } =
      projectManagementUpdateSchema.parse(requestBody);
    const projectManagement = await UpdateProjectManagement(
      params.projectID as string,
      { monitoringPlotsUrl, monitoringDataUrl, monitoringAdditionalFilesUrls, monitoringAdditionalFilesDescription }
    );
